...
Complete reference for all classes, functions, and decorators in the `kubiya.tools` module.
...
class Tool(BaseModel):
    ...
    with_volumes: Optional[List[Volume]] = []      # Volume mounts
    with_files: Optional[List[FileSpec]] = []        # File injections
    with_services: Optional[List[ServiceSpec]] = []     # Service dependencies
...
```python
from kubiya.tools import Tool

tool = Tool(
    name="example",
    description="Example tool",
    args=[
        Arg(name="count", type="int", required=True, description="Number of times to repeat"),
        Arg(name="message",  default="hello", type="str", required=False, description="Message to repeat"),
    ]
)
...
```python
from kubiya.tools import Tool, Arg, Volume, ServiceSpec
...
web_tool = Tool(
    ...
    args=[
        Arg(name="url",  description="Target URL", required=True),
...
database_tool = Tool(
    ...
    with_volumes=[
        Volume(path="/host/backups", name="/backups")
    ],
    with_services=[
        ServiceSpec(name="redis", image="redis:alpine", exposed_ports=[6379])
    ],
...
**Note**: Type specifications are optional for string arguments (they default to string). For other types (`int`, `float`, `bool`, `array`), specify the type to enable automatic validation and conversion.

```python
# String argument (type can be omitted, defaults to string)
url_arg = Arg(
    name="url",
...
# String with options (type can be specified or omitted)
level_arg = Arg(
    name="level",
    type="str",
...
```python
from kubiya.tools import FunctionTool
...
```python
from kubiya.tools import FunctionTool, Source
...
```python
from kubiya.tools import tool_registry

class ToolRegistry:
    """Global tool registry."""
    
    def register(self, tool: Tool) -> None:
        """Register a tool with default source."""
    
    def register_tool(self, source: str, tool: Tool):
        """Register a tool with a given source."""
...
```python
from kubiya.tools import tool_registry, Tool

my_tool = Tool(name="example", description="Example tool")
tool_registry.register(my_tool)
```

**`get_tool(source: str, name: str) -> Optional[Tool]`**

Retrieve a tool by source and name.

```python
tool = tool_registry.get_tool("default", "example")
if tool:
    print(f"Found tool: {tool.description}")
```

**`list_tools(source: str) -> List[Tool]`**

Get all registered tools from a source.

```python
all_tools = tool_registry.list_tools("default")
...
```python
from kubiya.tools import tool_registry, function_tool
...
# Get a specific tool
calculator = tool_registry.get_tool("default", "add")
if calculator:
...
mult_tool = tool_registry.get_tool("default", "multiply")
...
class Volume(BaseModel):
    path: str           # Host path  
    name: str           # Volume name/mount point
```
...
class FileSpec(BaseModel):
    source: Optional[str] = None    # File source
...
class ServiceSpec(BaseModel):
    ...
    entrypoint: Optional[List[str]] = []    # Container entrypoint
    exposed_ports: Optional[List[int]] = [] # Exposed ports
    volumes: Optional[List[Volume]] = []    # Volume mounts
```
...
tool = Tool(
    name="example",
    description="Example tool for type conversion",
    args=[
        Arg(name="count", type="int", description="Number of items to process", required=True),
        Arg(name="rate", type="float", description="Processing rate multiplier", default=1.0),
        Arg(name="enabled", type="bool", description="Enable advanced processing", default=False),
        Arg(name="items", type="array", description="List of items to process", default=[])
    ]
)
