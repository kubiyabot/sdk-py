...
simple_tool = Tool(
    ...
    args=[
        Arg(name="message",  description="Message to echo", required=True)
    ],
...
kubectl_tool = Tool(
    ...
    args=[
        Arg(name="command",  description="Kubectl command", required=True),
        Arg(name="namespace",  description="Namespace", default="default"),
...
aws_tool = Tool(
    ...
    args=[
        Arg(name="source_path",  description="Source path", required=True),
        Arg(name="bucket",  description="S3 bucket name", required=True),
        Arg(name="prefix",  description="S3 prefix", default=""),
...
database_tool = Tool(
    ...
    args=[
        ...
        Arg(name="backup_name",  description="Backup filename", required=True)
    ],
...
```python
from kubiya.tools import Tool, Arg, Volume
...
file_processor = Tool(
    ...
    args=[
        Arg(name="log_file",  description="Log file path", required=True),
        Arg(name="pattern",  description="Search pattern", required=True)
    ],
    with_volumes=[
        Volume(name="/host/logs", path="/logs"),
        Volume(name="/host/output", path="/output")
    ],
    content="""
#!/bin/bash
...
config_driven_tool = Tool(
    ...
    args=[
        Arg(name="action",  description="Terraform action", 
            options=["plan", "apply", "destroy"], required=True),
        Arg(name="workspace",  description="Terraform workspace", default="default")
    ],
...
```python
from kubiya.tools import Tool, Arg, Volume, FileSpec
...
cicd_tool = Tool(
    ...
    args=[
        Arg(name="repo_url",  description="Git repository URL", required=True),
        Arg(name="branch",  description="Git branch", default="main"),
        Arg(name="environment",  description="Target environment", 
    ...
    with_volumes=[
        Volume(name="/var/run/docker.sock", path="/var/run/docker.sock"),
        Volume(name="/host/workspace", path="/workspace")
    ],
    with_files=[
        FileSpec(
...
tool_registry.register_tool("default", monitoring_tool)
```
