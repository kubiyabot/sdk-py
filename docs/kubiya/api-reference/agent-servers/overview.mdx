...
<Note>
  Agent Server APIs are provided by orchestration servers built with the Kubiya SDK. These servers expose agent capabilities for intelligent workflow generation, execution, and orchestration. This includes both your own self-hosted Agent Servers and Kubiya's built-in Cloud orchestration server.
</Note>
