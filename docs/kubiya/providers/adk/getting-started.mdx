...
<Note>
  The ADK (Agent Development Kit) provider enables AI-powered workflow generation using advanced language models. It's the first official provider for the Kubiya SDK.
</Note>

## What is ADK Provider?

The ADK provider integrates Google's Agent Development Kit with the Kubiya SDK to enable:
...
<Steps>
  <Step title="Install SDK with ADK">
    ```bash
    pip install kubiya-sdk[adk]
    ```
  </Step>
  
  <Step title="Get API Keys">
    You'll need:
    - **Kubiya API Key**: From [compose.kubiya.ai](https://compose.kubiya.ai)
...
<AccordionGroup>
  <Accordion title="API Key Errors" icon="key">
    ...
    # Verify keys work
    python -c "from kubiya_sdk import Client; print(Client().get_organization_info())"
    ```
  </Accordion>
